...
html += """            </ol>
        ...
        <img src="graficos_finales/cluster_analysis.png" alt="Cluster Analysis" onclick="openModal(this)">
...
html += f"""            </div>
            ...
            <div class="projection-column">
                <div class="projection-result">
                    ...
                    <canvas id="projectionChart" width="450" height="350" style="max-width: 100%; margin: 15px 0; border: 1px solid #ddd;"></canvas>
...
html += """            </ul>
    ...
    <script>
        ...
        function drawChart(baseline, projection) {
            ...
            const baseY = marginTop + chartHeight / 2;  // Center line
            const centerX = marginLeft + chartWidth / 2;

            const barWidth = 100;
            const scale = chartHeight / 8;  // Scale for ±4% range

            // Draw title
            ctx.fillStyle = '#000';
            ctx.font = 'bold 13px Georgia';
            ctx.textAlign = 'center';
            ctx.fillText('GDP Growth Comparison', canvas.width / 2, 20);
            ...
            ctx.fillRect(centerX - barWidth - 30, baselineY, barWidth, Math.max(baselineHeight, 2));

            // Draw projection bar (orange)
            const projHeight = Math.abs(projection * scale);
            const projY = projection >= 0 ? baseY - projHeight : baseY;
            ctx.fillStyle = '#FF8C00';
            ctx.fillRect(centerX + 30, projY, barWidth, Math.max(projHeight, 2));

            // Labels below bars
            ctx.fillStyle = '#000';
            ctx.font = 'bold 12px Georgia';
            ctx.textAlign = 'center';
            ctx.fillText('Baseline', centerX - barWidth/2 - 30, baseY + 35);
            ctx.fillText('Your Scenario', centerX + barWidth/2 + 30, baseY + 35);

            // Values below labels
            ctx.font = '13px Georgia';
            ctx.fillStyle = '#4682B4';
            ctx.fillText(baseline.toFixed(2) + '%', centerX - barWidth/2 - 30, baseY + 53);
            ctx.fillStyle = '#FF8C00';
            ctx.fillText(projection.toFixed(2) + '%', centerX + barWidth/2 + 30, baseY + 53);
        }
